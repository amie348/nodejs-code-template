...
const validateInput = (inputSchema, schemaScope) => {
  ...
  const requestBodyValidator = async (req, res, next) => {
    ...
    try {
      ...
      const errorDescription = `${(error.details.map((entry, i) => `${entry.message.replace(/"/g, `'`)}${!entry.message.endsWith(`.`) ? `.` : ``}`).join(` `))}`;
  ...
  const requestQueryValidator = async (req, res, next) => {
    ...
    try {
      ...
      if (!Object.keys(req.query).length) {
        ...
        // logging error message to the console
        logError(`Incoming request query can't be empty.`);
      ...
      // validating the incoming schema
      const validationResult = await inputSchema.validateAsync(req.query, { abortEarly: false });
  ...
  return schemaScope.toUpperCase() === BODY ? requestBodyValidator : schemaScope.toUpperCase() === PARAMS ? requestPathParamsValidator : schemaScope.toUpperCase() === QUERY ? requestQueryValidator : requestDummyValidator;
